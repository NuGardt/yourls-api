...
Namespace Yourls.API.UnitTest.CreateShortUrl
  Public Class CreateShortUrlBeginUnitTest
    ...
    Public Sub Start(ByVal OnCompletion As AsyncCallback,
      ...
      Else
        Call Me.Service.CreateShortUrlBegin(Key := Nothing, LongUrl := "http://www.nugardt.com", Authentication := MyAuth, Callback := EndCallback, Keyword := "", Title := "")
      End If
    End Sub
    ...
    Public Sub [End](Optional Result As IAsyncResult = Nothing) Implements IUnitTestCase.[End]
      ...
      Me.Ex = Me.Service.CreateShortUrlEnd(Result, Nothing, Response)

      If (Ex Is Nothing) Then
        If Response.HasError Then
          Me.Ex = Response.ToException()
        Else
          Me.m_Result = Helper(Of YourlsCreateShortUrlResult).CheckResult("CreateShortUrlBegin", Me.Ex, Response, TreatSoftFailAsFail := False)
        End If
      End If
    End Sub
    ...
    Public ReadOnly Property Name As String Implements IUnitTestCase.Name
      Get
        Return "YOURLS API: CreateShortUrlBegin"
      End Get
    End Property

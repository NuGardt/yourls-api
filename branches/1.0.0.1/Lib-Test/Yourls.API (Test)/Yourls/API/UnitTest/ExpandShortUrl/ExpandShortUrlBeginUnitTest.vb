...
Namespace Yourls.API.UnitTest.ExpandShortUrl
  Public Class ExpandShortUrlBeginUnitTest
    ...
    Public Sub Start(ByVal OnCompletion As AsyncCallback,
      ...
      Else
        Call Me.Service.ExpandShortUrlBegin(Key := Nothing, ShortUrl := "http://ls.nugardt.com/2go8s", Authentication := MyAuth, Callback := EndCallback)
      End If
    End Sub
    ...
    Public Sub [End](Optional Result As IAsyncResult = Nothing) Implements IUnitTestCase.[End]
      ...
      Me.Ex = Me.Service.ExpandShortUrlEnd(Result, Nothing, Response)

      If (Ex Is Nothing) Then
        If Response.HasError Then
          Me.Ex = Response.ToException()
        Else
          Me.m_Result = Helper(Of YourlsExpandShortUrlResult).CheckResult("ExpandShortUrlBegin", Me.Ex, Response, TreatSoftFailAsFail := False)
        End If
      End If
    End Sub
    ...
    Public ReadOnly Property Name As String Implements IUnitTestCase.Name
      Get
        Return "YOURLS API: ExpandShortUrlBegin"
      End Get
    End Property
